...
namespace PLD
{
	...
	public abstract class ParticleSystem : DrawableGameComponent
	{
		...
		protected ParticleSystem(LemmingDeath game, int howManyEffects)
			: base(game)
		{            
			this.game = game;
		...
		public void AddParticles(Vector2 where)
		{
			...
			int numParticles =
				LemmingDeath.Random.Next(minNumParticles, maxNumParticles);
		...
		protected virtual void InitializeParticle(Particle p, Vector2 where)
		{
			...
			float velocity =
				LemmingDeath.RandomBetween(minInitialSpeed, maxInitialSpeed);
			float acceleration =
				LemmingDeath.RandomBetween(minAcceleration, maxAcceleration);
			float lifetime =
				LemmingDeath.RandomBetween(minLifetime, maxLifetime);
			float scale =
				LemmingDeath.RandomBetween(minScale, maxScale);
			float rotationSpeed =
				LemmingDeath.RandomBetween(minRotationSpeed, maxRotationSpeed);
		...
		{
			float angle = LemmingDeath.RandomBetween(0, MathHelper.TwoPi);
